...
public class test {
    ...
    public static void cmpStructuralCoefficients() throws QueueException, Exception
    {
        ...
        queue.add(lprj1);            
        CINCAMIPD lprj2=(CINCAMIPD)TranslateJSON.toObject(CINCAMIPD.class,jsonPD2);
        queue.add(lprj2);                       
        ...
        //Original Computing for the Structural Coefficient (Common Attributes)
        MeasurementProject prj1=lprj1.getProjects().getProjects().get(0);
        MeasurementProject prj2=lprj2.getProjects().getProjects().get(0);
        
        Attributes atprj1=prj1.getInfneed().getSpecifiedEC().getDescribedBy();
        java.util.HashMap<String,Attribute> mprj1=new java.util.HashMap(atprj1.getCharacteristics().size()+
                prj1.getInfneed().getCharacterizedBy().getDescribedBy().getContextProperties().size());
        System.out.println("***PRJ1***");
        System.out.println("ID\tName\tDefinition");
        for(Attribute at:atprj1.getCharacteristics())
        {
            mprj1.put(at.getID(), at);
            System.out.println(at.getID()+"\t"+at.getName()+"\t"+at.getDefinition());
        }
        for(Attribute at:prj1.getInfneed().getCharacterizedBy().getDescribedBy().getContextProperties())
        {
            mprj1.put(at.getID(), at);
            System.out.println(at.getID()+"\t"+at.getName()+"\t"+at.getDefinition()+" ***Context Properties***");
        }

        Attributes atprj2=prj2.getInfneed().getSpecifiedEC().getDescribedBy();
        java.util.HashMap<String,Attribute> mprj2=new java.util.HashMap(atprj2.getCharacteristics().size()+
                prj2.getInfneed().getCharacterizedBy().getDescribedBy().getContextProperties().size());
        System.out.println("***PRJ2***");
        System.out.println("ID\tName\tDefinition");        
        for(Attribute at:atprj2.getCharacteristics())
        {
            mprj2.put(at.getID(), at);
            System.out.println(at.getID()+"\t"+at.getName()+"\t"+at.getDefinition());
        }
        for(Attribute at:prj2.getInfneed().getCharacterizedBy().getDescribedBy().getContextProperties())
        {
            mprj2.put(at.getID(), at);
            System.out.println(at.getID()+"\t"+at.getName()+"\t"+at.getDefinition()+" ***Context Properties***");
        }
        ...
        int common=0;
        java.util.Iterator<String> lkey1=mprj1.keySet().iterator();
        while(lkey1.hasNext())
        {
            String key=lkey1.next();
            if(mprj2.containsKey(key)) 
            {
                common++;
                mprj2.remove(key);
                mprj1.remove(key);
        ...
        int justPrj1=mprj1.size();
        int justPrj2=mprj2.size();
        double oss=((double)common)/(((double)common)+((double)justPrj1)+((double)justPrj2));
        System.out.println("Original Structural Similarity: "+oss);
        
        double simacu=0.0;
        //NEW: The New Text Similarity-Driven Structural Coefficient
        System.out.println("***The New Text Similarity-Driven Structural Coefficient***");
        ArrayList all1=new ArrayList();
        all1.addAll(atprj1.getCharacteristics());
        all1.addAll(prj1.getInfneed().getCharacterizedBy().getDescribedBy().getContextProperties());
        ArrayList all2=new ArrayList();
        all2.addAll(atprj2.getCharacteristics());
        all2.addAll(prj2.getInfneed().getCharacterizedBy().getDescribedBy().getContextProperties());
        double threshold=0.6;
        common=0;
        for(Object aot1:all1)
        {
            Attribute at1=(Attribute)aot1;
            boolean ready=false;
            for(Object aot2:all2)
            {
                
               Attribute at2=(Attribute)aot2;
               int posat1=da.getPosition(at1.getID());
               int posat2=da.getPosition(at2.getID());
               
               double cmp=matrix.get(posat1, posat2);

               if(!ready && cmp>=threshold)
               {                              
                System.out.println("At.PRJ1: "+at1.getID()+" At.PRJ2: "+at2.getID()
                       +" Individual Similarity: "+matrix.get(posat1, posat2) + " **Equivalent**");
                common++;               
                ready=true;
               }
               else
               {
                System.out.println("At.PRJ1: "+at1.getID()+" At.PRJ2: "+at2.getID()
                       +" Individual Similarity: "+matrix.get(posat1, posat2));
               }
            }
        }                
        
        System.out.println("Total common: "+(common));
        System.out.println("Total Attributes among both projects: "+((all1.size()+all2.size())));
        System.out.println("New Structural Coef: "+((double)common/(double)(all1.size()+all2.size()-common)));
        
        System.out.println("By DA Method***: "+da.structuralCoef(prj1, prj2, 0.6));
    }
